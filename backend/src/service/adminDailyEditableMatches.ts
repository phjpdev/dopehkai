...
export function isMatchAdminDailyEditable(
    matchId: string,
    kickOff: string | undefined | null,
    list: Array<{ id?: string; eventId?: string; kickOff?: string }>
): boolean {
    const id = String(matchId);
    const dayKey = kickOffDateKeyHongKong(kickOff || "");
    if (!dayKey) return false;
    const sameDay = list
        .map((m) => ({ id: String(m.id || m.eventId || ""), k: m.kickOff }))
        .filter((x) => x.id && kickOffDateKeyHongKong(x.k) === dayKey)
        .map((x) => x.id);
    const sorted = [...new Set(sameDay)].sort((a, b) => a.localeCompare(b));
    return pickTwoDistinctSortedIds(sorted, dayKey).includes(id);
}
