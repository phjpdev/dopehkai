...
function Records2AdminPage() {
    const { t } = useTranslation();
    const queryClient = useQueryClient();
    const [page, setPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const [sorting, setSorting] = useState<SortingState>([]);
    const { data } = useRecords2(page, pageSize);
        ...
        if (validateForm()) {
            ...
            try {
                if (editId) {
                    ...
                    formData.append("table", "records2");
                    for (const file of r.media) {
                        let compressed = file;
                        formData.append("media", compressed);
                    }
                    const res = await API.POSTFORMDATA(`${AppGlobal.baseURL}records2/${editId}`, formData);
                    if (res.status == 200) {
                        queryClient.invalidateQueries({ queryKey: ["records2"] });
                    ...
                    const res = await API.POSTFORMDATA(`${AppGlobal.baseURL}records2`, formData);
                    if (res.status == 200) {
                        queryClient.invalidateQueries({ queryKey: ["records2"] });
    ...
    const handleOnDelete = async () => {
        const res = await API.DELETE(`${AppGlobal.baseURL}records2/${deleteId}?table=records2`);
        if (res.status == 200) {
queryClient.invalidateQueries({ queryKey: ["records2"] });
        } else if (res.status == 409) {
            alert(res.data.error)
...
export default Records2AdminPage;
